...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"动态高度带收缩";
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //因为复用性问题展开按钮有时会显示不出来，所以不复用
//    DiscoverTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    
//    if (cell == nil) {
    
    DiscoverTableViewCell *cell = [[DiscoverTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
//    }
    cell.delegate = self;
    
    [cell setData:_dataArray[indexPath.row] andIndexRow:indexPath.row andType:_typeArray[indexPath.row]];
    
    return cell;

}

#pragma mark -- DiscoverDelegate
-(void)disCoverBtnClick:(NSDictionary *)dict{

    //获取当前cell的indexPath.row
    NSString *indexStr = @"";
    if ([dict allKeys].count > 0) {
        indexStr = [dict allKeys].firstObject;
    }
    
    //获取当前cell的状态（展开或者收缩）
    NSString *typeStr = @"";
    if ([dict allValues].count > 0) {
        typeStr = [dict allValues].firstObject;
    }

    //删除初始的状态记录
    [self.typeArray removeObjectAtIndex:[indexStr intValue]];
    //插入改变后的状态记录
    [self.typeArray insertObject:typeStr atIndex:[indexStr intValue]];
